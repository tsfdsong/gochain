	...
	go func() {
		for {
			...
			case v := <-pp.chain.SyncBlockChan:
				...
				// log.Printf("*********  on incoming block: %v  %v   %v  %v\n", v.BlockNum, v.Producer, v.TimeStamp.Time, v.ID.String())
...
func (pp *ProducerPlugin) scheduleProductionLoop() {
	...
	if result == failed {
		...
		if result == succeeded {
			pslot := time.Duration(pp.chain.PendingBlockState().Header.TimeStamp.Time.Int64())
			cslot := time.Duration(time.Now().UnixNano()) / time.Millisecond
			tsub := pslot - cslot
			if tsub > 0 {
				delaytime := tsub * time.Millisecond
...
func (pp *ProducerPlugin) startBlock() uint8 {
	...
	curtime := uint64(base.UnixNano() / int64(time.Millisecond))
	mintimenextblock := types.BlockIntervalMs - curtime%types.BlockIntervalMs
	...
	if blocktime.Sub(now).Nanoseconds() < interv {
		log.Print("Producer StartBlock wait for the whole block interval.")
...
func (pp *ProducerPlugin) produceBlock() {
	...
	if !bfound {
		log.Printf("Producer not on turn for :%v\n", proname)
		return
	}
...
func (pp *ProducerPlugin) OnIncomingBlock(block *types.Block) {
	...
	hbs := pp.chain.HeadBlockState().Header.TimeStamp.Time.Int64() + int64(types.BlockIntervalMs)
	sb := hbs - time.Now().UnixNano()/int64(time.Millisecond)
