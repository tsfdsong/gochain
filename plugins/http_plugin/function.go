...
//sort transactions by blockid
type BSlice []*TransactionDetail

func (c BSlice) Len() int {
	return len(c)
}
func (c BSlice) Swap(i, j int) {
	c[i], c[j] = c[j], c[i]
}
func (c BSlice) Less(i, j int) bool {
	return c[i].BlockHeight < c[j].BlockHeight
}
...
func FindTransaction() ([]*TransactionDetail, error) {
	...
	for _, val := range index {
		...
		result.BlockHeight = int64(val.BlockHeaderState.BlockNum)

		t := val.BlockHeaderState.Header.TimeStamp.Time.Int64() / 1000
		result.TimeStamp = time.Unix(t, 0).String()

		for _, v := range val.Block.Transactions {
			var trxresu types.Transaction
			err := rlp.DecodeBytes(v.PackedTrx.PackedTrx, &trxresu)
			if err != nil {
				return nil, err
			}
			for _, action := range trxresu.Actions {
				data := action.Data
				if err := json.Unmarshal(data, &transfer); err == nil {
					result.Amount += float64(transfer.Amount)
					result.TrxHash = append(result.TrxHash, trxresu.TransactionHash.String())
...
func QueryTransactionById(id common.Hash) *TransactionDetail {
	...
	for _, val := range index {
		...
		for _, v := range val.Trxs {
			if v.ID == id {
				result.TrxHash = append(result.TrxHash, v.ID.String())
...
func QueryBlockByID(id common.Hash) *BlockByIDResult {
	...
	ts := blockstate.BlockHeaderState.Header.TimeStamp.Time.Int64() / 1000
