...
type P2pPluginConfig struct {
	...
	StaticNodes     []string `yaml:"StaticNodes"`
}
...
func (p *P2p_Plugin) InitWithConfig(p2pConfig *P2pPluginConfig) error {
	...
	nodes := make([]*discover.Node, len(p2pConfig.StaticNodes))
	nodes = nodes[0:0]

	for _, staticNode := range p2pConfig.StaticNodes {
		node, err := discover.ParseNode(staticNode)
		if err != nil {
	...
	log.Printf("static node len:%v", len(nodes))
