...
func NewBlockTime(t time.Time) *BlockTime {
	...
	res.Time = big.NewInt(t.UnixNano() / int64(time.Millisecond))
	res.Slot = (res.Time.Uint64() - res.Epoch) / res.Interval
...
func (bt *BlockTime) SetTime(t time.Time) {
	bt.Time = big.NewInt(t.UnixNano() / int64(time.Millisecond))

	bt.Slot = (bt.Time.Uint64() - bt.Epoch) / bt.Interval
}
