...
func (fdb *ForkDB) insert(b *types.BlockState) bool {
	...
	if _, ok := fdb.index[b.Block.ID]; ok {
		return false
	}

	fdb.index[b.Block.ID] = *b
...
func (fdb *ForkDB) AddBlock(b *types.Block, trust bool) *types.BlockState {
	...
	if !ok {
		log.Printf("****AddBlock Failed: %v  %v\n", b.BlockNum, b.Previous.String())
		return nil
	}

	// log.Printf("****AddBlock: %v  %v  %v\n", b.BlockNum, b.Producer, b.ID.String())
