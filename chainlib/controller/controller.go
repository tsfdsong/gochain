...
func (cp *Controller) CommitBlock(addtoforkdb bool) {
	...
	if addtoforkdb {
		...
		log.Print("**************\n")
		log.Printf("commit block-1 : %v   %v  %v\n", cp.Pending.PendingBlockState.Block.Producer, cp.Pending.PendingBlockState.Block.BlockNum, cp.Pending.PendingBlockState.Trxs)
		log.Printf("commit block-2 : %v  %v  %v\n", cp.Pending.PendingBlockState.Block.BlockHeader.TimeStamp.Time.Int64(), cp.Pending.PendingBlockState.Block.BlockHeader.TimeStamp.Slot, cp.Pending.PendingBlockState.Block.ID.String())
		log.Print("**************\n")
...
func (cp *Controller) HeadBlockTime() time.Time {
	return time.Unix(0, cp.Head.Header.TimeStamp.Time.Int64()*int64(time.Millisecond))
}
